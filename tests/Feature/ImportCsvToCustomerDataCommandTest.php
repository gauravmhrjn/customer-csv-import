...
final class ImportCsvToCustomerDataCommandTest extends TestCase
{
    public function test_it_runs_successfully(): void
    {
        // Act & Assert
        $this->artisan('app:import-csv')
            ->assertSuccessful()
            ->expectsOutput();
    }
    ...
    public function test_it_can_parse_the_csv_file_to_extract_and_parse_the_customer_names_into_expected_format(): void
    {
        ...
        // Act & Assert
        $command = $this->withoutMockingConsoleOutput()->artisan('app:import-csv');

        $commandOutput = Artisan::output();
        $parsedData = json_decode($commandOutput, true);
    ...
    public function test_it_returns_error_when_csv_file_is_missing(): void
    {
        ...
        $this->artisan('app:import-csv')
            ->assertFailed()
            ->expectsOutput(CsvFileNotFoundException::ERROR_MESSAGE);
    }
}
